...
Пример отправляет один и тот же текстовый запрос в LLM через GigaChat API два раза:

- без ограничений;
- с явным форматом ответа, ограничением длины и stop sequence.

После этого CLI печатает оба ответа в консоль, чтобы их можно было сравнить.
...
```text
Запрос:
Объясни, что такое API, одним предложением
...
- явный формат ответа: JSON с полями `answer` и `key_points`;
- ограничение длины: `max_tokens: 120`;
- условие завершения: stop sequence `###END###`.
