...
Число запусков растет, потому что MCP-сервер выполняет задачу по расписанию.
...
Агент подключается к MCP, создает периодическую сводку, показывает агрегированный результат
и умеет отключать задачу по числовому id.
```
